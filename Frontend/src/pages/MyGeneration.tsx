        ...
        {loading && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
           {Array.from({length:6}).map((_,index)=>(
              <div key={index} className="rounded-2xl bg-white/6 border border-white/10 animate-pulse h-[260px] "></div>
            ))}
          </div>
        )}
          {/* EMPTY STATE */}
                    ...
                    <div key={thumb._id} onClick={()=>navigate(`/generate/${thumb._id}`)} className="mb-8 group relative cursor-pointer rounded-2xl bg-white/6 border border-white/10 transition shadow-xl break-inside-avoid">
                        ...
                        <div className="p-4 space-y-2">
                          ...
                          <div className="flex flex-wrap gap-2 text-xs text-zinc-400">
                            <span className="inline-block px-2 py-1 text-xs rounded-md bg-white/10 text-zinc-300">
                              {thumb.style}
                            </span>
                            <span className="inline-block px-2 py-1 text-xs rounded-md bg-white/10 text-zinc-300">
                              {thumb.color_scheme}
